...
class OTAAssigner_gtiou(BaseAssigner):
    ...
    def assign(self,
        ...
        gt_ious = self.iou_calculator(gt_bboxes, gt_bboxes)
        ...
        matched_pred_ious, matched_gt_inds , dynamic_ks, topk_ious, top_index = \
            self.dynamic_k_matching(
                cost_matrix, pairwise_ious, gt_ious,  num_gt, valid_mask)
    ...
    def dynamic_k_matching(self, cost: Tensor, pairwise_ious: Tensor, gt_ious :Tensor,
                           num_gt: int,
        ...
        topk_ious, top_index = torch.topk(pairwise_ious, candidate_topk, dim=0)   #各gtについてiouがtopkのanchorとのiou
        # calculate dynamic k for each gt
        dynamic_ks = torch.clamp((topk_ious.sum(0)-gt_ious.sum(0)).int(), min=1) #各gtの割り当てanchor数.min(topk_ious.sum(0), 1)
        ...
        return matched_pred_ious, matched_gt_inds, dynamic_ks, topk_ious, top_index
