...
class OTAAssigner_custom(BaseAssigner):
    ...
    def assign(self,
        ...
        pairwise_GIous = pairwise_ious - self.GIou(valid_decoded_bbox, gt_bboxes)
        ...
        valid_mask_iou = valid_mask.detach().numpy().copy()
        matched_pred_ious, matched_gt_inds , dynamic_ks, topk_ious, top_index = \
            self.dynamic_k_matching(
                cost_matrix, pairwise_ious, num_gt, valid_mask, pairwise_GIous)
        ...
        return AssignResult(
            num_gt, assigned_gt_inds, max_overlaps, labels=assigned_labels), dynamic_ks, topk_ious, top_index, valid_mask_iou, []
    ...
    def dynamic_k_matching(self, cost: Tensor, pairwise_ious: Tensor,
                           num_gt: int,
                           valid_mask: Tensor,
                           pairwise_Gious: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        topk_gious, top_index = torch.topk(pairwise_Gious, candidate_topk, dim=0)
        topk_ious, _ = torch.topk(pairwise_ious, candidate_topk, dim=0)   #各gtについてiouがtopkのanchorとのiou
        # calculate dynamic k for each gt
        topk_iou_var = topk_ious.var(dim=0)
        topk_giou_var = topk_gious.var(dim=0)
        dynamic_ks = torch.clamp((topk_gious.sum(0) - topk_giou_var * 100).int()*2, min=1) #各gtの割り当てanchor数.min(topk_ious.sum(0), 1)
        ...
        return matched_pred_ious, matched_gt_inds, dynamic_ks, topk_gious, top_index
    
    def GIou(self, bboxes, gt):
