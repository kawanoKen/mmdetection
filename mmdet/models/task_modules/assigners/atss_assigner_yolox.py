...
class ATSSAssigner_yolox(BaseAssigner):
    ...
    def assign(
        ...
        num_level_priors = [num_priors * 16//21, num_priors * 4//21, num_priors //21]
        INF = 100000000
        priors[:, 0] = priors[:, 0] - priors[:, 2] // 2
        priors[:, 1] = priors[:, 1] - priors[:, 3] // 2
        priors[:, 2] = priors[:, 0] + priors[:, 2]
        priors[:, 3] = priors[:, 1] + priors[:, 3]
...
class ATSSAssigner_yolox_test(BaseAssigner):
    """Assign a corresponding gt bbox or background to each prior.
