...
simota_valid_index = []
for i in range(8400):
    if ota_valid[img_num][i]:
        ota_valid_index.append(i)
    if simota_valid[img_num][i]:
        simota_valid_index.append(i)
for i in range(num_gt):
    _, pos_idx = torch.topk(ota_cost_matrix[img_num][:, i], k=10, largest=False)
    ota_top10_idxes.append([ota_valid_index[pos_idx[j]]for j in range(10)])
for i in range(num_gt):
    _, pos_idx = torch.topk(simota_cost_matrix[img_num][:, i], k=10, largest=False)
    simota_top10_idxes.append([simota_valid_index[pos_idx[j]]for j in range(10)])
...
for i in range(num_gt):
    ...
    top10_bboxes_preds = [ota_decoded_bboxes[img_num][ota_top10_idxes[i][j]].detach().numpy()for j in range(10)]
    ...
    for bb in assigned_bboxes_preds:
        ...
        w = bb[2] - bb[0]
        h = bb[3] - bb[1]
        r = patches.Rectangle( xy=(x,y) , width=w, height=h, color=(0, 1, 0), fill=False) # 四角形のオブジェクト
        ax.add_patch(r)

    for bb in top10_bboxes_preds:
        x = bb[0]
        y = bb[1]
        w = bb[2] - bb[0]
        h = bb[3] - bb[1]
        r = patches.Rectangle( xy=(x,y) , width=w, height=h,color=(0, 0, 1), fill=False) # 四角形のオブジェクト
        ax.add_patch(r)
    ...
    top10_bboxes_preds = [simota_decoded_bboxes[img_num][simota_top10_idxes[i][j]].detach().numpy()for j in range(10)]
...
plt.show()
plt.savefig("ota(L)_vs_simota(R)_50e_top10show_2ndimg.jpg") 
