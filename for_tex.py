...
for v in runner.train_dataloader:
    ...
    checkpoint_path = "/data1/kawano/ota/mmdetection_1/work_dirs/yolox_s_simota_8xb8-300e_coco/epoch_10.pth"
    load_checkpoint(yolox, checkpoint_path)
    cls_scores, bbox_preds, objectnesses = yolox(inputs)
...
img_array = [[[img_array[2][j][k], img_array[1][j][k], img_array[0][j][k]] for k in range(640)] for j in range(640)]
...
ax = fig.add_subplot(2, 2, 1, title = f"OTAAssigner_10e,  gt_label = {1}, dynamic_k = {k}")
ax.imshow(img_array)
ax.set_xticks(ticks)
ax.set_yticks(ticks)
ax.grid()
...
ax = fig.add_subplot(2, 2, 2, title = f"SimOTAAssigner_10e, dynamic_k = {k}, num_assign = {n}")
ax.imshow(img_array)
ax.set_xticks(ticks)
ax.set_yticks(ticks)
ax.grid()
...
k = ota_dynamic_ks[img_num][i]
l = gt_instances[img_num]["labels"][i]
ax = fig.add_subplot(2, 2, 3, title = f"gt_label = {2}, dynamic_k = {k}")
ax.imshow(img_array)
ax.set_xticks(ticks)
ax.set_yticks(ticks)
ax.grid()
...
k = simota_dynamic_ks[img_num][i]
n = assigned_anchors.sum()
ax = fig.add_subplot(2, 2,4, title = f"dynamic_k = {k}, num_assign = {n}")
ax.imshow(img_array)
ax.set_xticks(ticks)
ax.set_yticks(ticks)
ax.grid()
...
plt.show()
plt.savefig("tex_ota_vs_simota_10e_8vs10.jpg")
