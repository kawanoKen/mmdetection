...
from mmdet.models.task_modules.assigners import SimOTAAssigner_test, OTAAssigner_test, OTAAssigner_custom
import numpy as np
...
for v in runner.train_dataloader:
    ...
    checkpoint_path = "/data3/yamamura/mmdet_custom/mmdetection-kawano/work_dirs/yolox_s_ota_8xb8-300e_coco/epoch_10.pth"
    ...
    yolox.bbox_head.assigner = OTAAssigner_test(candidate_topk = 8)
    load_checkpoint(yolox, checkpoint_path)
    cls_scores, bbox_preds, objectnesses = yolox(inputs)
    simota_results, simota_decoded_bboxes, simota_cls_preds, simota_dynamic_ks, simota_topk_ious, simota_topk_index, simota_valid, _ = yolox.bbox_head.assigner_test(cls_scores, bbox_preds, objectnesses, gt_instances, 8)
    break
...
ota_cls_softmax = m(ota_cls_preds[1])
simota_cls_softmax = m(simota_cls_preds[1])
...
# for i in range(8400):
#     if ota_valid[1][i]:
#         ota_valid_index.append(i)
#     if simota_valid[1][i]:
#         simota_valid_index.append(i)

# ota_valid_topk_index = [[ota_valid_index[ota_topk_index[img_num][i][j]] for i in range(10)] for j in range(16)]
# simota_valid_topk_index = [[simota_valid_index[simota_topk_index[img_num][i][j]] for i in range(10)] for j in range(16)]

# print(simota_topk_ious[1][: ,7])
# print(simota_topk_ious[1][: ,9])
# print(np.array(simota_valid_topk_index)[7])
# print(np.array(simota_valid_topk_index)[9])
...
plt.show()
plt.savefig("ota(L)_vs_ota(R)_10e_2ndimg_k = 8.jpg") 
